...
| 文件 | 方法 |
|------|------|
| ML03_DoubleML | 双重机器学习（数字经济对劳动力市场的因果效应）|

### 三、深度学习（DL）

| 文件 | 方法 |
|------|------|
| DL08_MLP | 多层感知机回归（房价预测基准模型）|

### 四、强化学习（RL）

| 文件 | 方法 |
|------|------|
| RL01_QLearning | Q-Learning 投资组合再平衡 |

### 五、大语言模型（LLM）

| 文件 | 方法 |
|------|------|
| LLM01_GPT情感分析 | 年报情感分析与超额收益 |
...
│   ├── ml/                      # DoubleML / CausalForest / Lasso / BART / HighDimIV
│   ├── dl/                      # MLP（PyTorch）
│   ├── rl/                      # Q-Learning（Gymnasium）
│   ├── llm/                     # GPT调用 / 情感分析 / RAG文献综述
...
print(DoubleML().fit(X, y, d).summary())
```
